...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <form className="grid gap-2 rounded border p-4 md:grid-cols-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {result.items.map((listing) => (
          <Link key={listing.id} href={`/listings/${listing.id}`} className="rounded border p-3">
            {listing.photos[0] && <img src={listing.photos[0].url} alt="" className="mb-2 h-40 w-full rounded object-cover" />}
            <h2 className="font-semibold">{listing.title}</h2>
            <p className="text-sm">{listing.author} · {listing.genre}</p>
            <p className="text-xs text-gray-500">{listing.owner.displayName} · {listing.owner.locationDistrict ?? "district unknown"} · relevance {listing.relevance.toFixed(3)}</p>
          </Link>
        ))}
      </div>
      {result.nextCursor && (

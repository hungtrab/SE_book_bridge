...
export function FeedList({ initial }: { initial: FeedRow[] }) {
  ...
  if (items.length === 0) {
    return <p className="card-surface rounded-2xl p-5 text-[color:var(--muted)]">Follow users or join communities to build your feed.</p>;
  }
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {items.map((item) => item.listing && (
        <Link key={item.id} href={`/listings/${item.listing.id}`} className={`card-surface interactive-card overflow-hidden rounded-2xl p-3 ${item.seen ? "" : "border-blue-500"}`}>
          {item.listing.photos[0] && <img src={item.listing.photos[0].url} alt="" className="mb-3 h-40 w-full rounded-xl object-cover" />}
          <h3 className="text-lg font-bold">{item.listing.title}</h3>
